...
func listActive() {
	...
	plansByProjectId := make(map[string][]*internal.Plan)
	...
	if len(currentProjectPlanIds) > 0 {
		...
		currentBranchesByPlanId, apiErr := api.Client.GetCurrentBranchByPlanId(lib.CurrentProjectId, internal.GetCurrentBranchByPlanIdRequest{
