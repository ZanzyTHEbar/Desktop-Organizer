...
var contextRmCmd = &cobra.Command{
	...
	desktop-cleaner plan rm 1 # Remove by index in the 'desktop-cleaner ls' list
	desktop-cleaner plan rm 1-3
	desktop-cleaner plan rm some-file.ts
	desktop-cleaner plan rm app/*.py
	`,
...
func init() {
	PlanCmd.AddCommand(contextRmCmd)
}
