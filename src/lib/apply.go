...
func MustApplyPlan(planId, branch string, autoConfirm bool) {
	...
	commitSummary, apiErr = api.Client.ApplyPlan(planId, branch, internal.ApplyPlanRequest{
		ApiKeys:     apiKeys,
	...
	if len(updatedFiles) == 0 {
		...
		if isRepo {
			fmt.Println("✏️  DesktopCleaner can commit these updates with an automatically generated message.")
			fmt.Println()
			fmt.Println("ℹ️  Only the files that DesktopCleaner is updating will be included the commit. Any other changes, staged or unstaged, will remain exactly as they are.")
			fmt.Println()

			confirmed, err := term.ConfirmYesNo("Commit DesktopCleaner updates now?")
