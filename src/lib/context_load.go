...
func MustLoadContext(resources []string, params *types.LoadContextParams) {
	...
	var loadContextReq internal.LoadContextRequest
	...
	if params.Note != "" {
		loadContextReq = append(loadContextReq, &internal.LoadContextParams{
			ContextType: internal.ContextNoteType,
			Body:        params.Note,
			ApiKeys:     apiKeys,
	...
	if fileInfo.Mode()&os.ModeNamedPipe != 0 {
		...
		if len(pipedData) > 0 {
			...
			loadContextReq = append(loadContextReq, &internal.LoadContextParams{
				ContextType: internal.ContextPipedDataType,
	...
	alreadyLoadedByComposite := make(map[string]*internal.Context)
	...
	existsByComposite := make(map[string]*internal.Context)
	for _, context := range existingContexts {
		switch context.ContextType {
		case internal.ContextFileType, internal.ContextDirectoryTreeType:
	...
	if len(inputFilePaths) > 0 {
		...
		if params.NamesOnly {
			for _, inputFilePath := range inputFilePaths {
				composite := strings.Join([]string{string(internal.ContextDirectoryTreeType), inputFilePath}, "|")
				...
				go func(inputFilePath string) {
					...
					loadContextReq = append(loadContextReq, &internal.LoadContextParams{
						ContextType:     internal.ContextDirectoryTreeType,
						Name:            name,
						Body:            body,
			...
			for _, path := range flattenedPaths {
				var contextType internal.ContextType
				isImage := internal.IsImageFile(path)
				if isImage {
					contextType = internal.ContextImageType
				} else {
					contextType = internal.ContextFileType
				}
	...
	if len(inputUrls) > 0 {
		for _, u := range inputUrls {
			composite := strings.Join([]string{string(internal.ContextURLType), u}, "|")
			if existsByComposite[composite] != nil {
				alreadyLoadedByComposite[composite] = existsByComposite[composite]
				continue
			}
			...
			go func(u string) {
				...
				loadContextReq = append(loadContextReq, &internal.LoadContextParams{
					ContextType: internal.ContextURLType,
					Name:        name,
					Body:        body,
					Url:         u,
				})
...
func printAlreadyLoadedMsg(alreadyLoadedByComposite map[string]*internal.Context) {
	fmt.Println()
	pronoun := "they're"
