...
func MustCheckOutdatedContext(quiet bool, maybeContexts []*internal.Context) (contextOutdated, updated bool) {
	if !quiet {
...
func MustUpdateContext(maybeUpdateContexts []*internal.Context) {
...
func UpdateContext(maybeContexts []*internal.Context) (*types.ContextOutdatedResult, error) {
	return checkOutdatedAndMaybeUpdateContext(true, maybeContexts)
}

func CheckOutdatedContext(maybeContexts []*internal.Context) (*types.ContextOutdatedResult, error) {
	return checkOutdatedAndMaybeUpdateContext(false, maybeContexts)
}

func checkOutdatedAndMaybeUpdateContext(doUpdate bool, maybeContexts []*internal.Context) (*types.ContextOutdatedResult, error) {
	...
	req := internal.UpdateContextRequest{}
	var updatedContexts []*internal.Context
	...
	contextsById := map[string]*internal.Context{}
	deleteIds := map[string]bool{}

	var paths *fs.CleanerPaths
	var hasDirectoryTreeWithIgnoredPaths bool

	for _, context := range contexts {
		if context.ContextType == internal.ContextDirectoryTreeType && !context.ForceSkipIgnore {
	...
	if hasDirectoryTreeWithIgnoredPaths {
		...
		var err error
		paths, err = fs.GetCleanerPaths(baseDir)
		if err != nil {
	...
	for _, context := range contexts {
		...
		if context.ContextType == internal.ContextFileType {
			wg.Add(1)
			go func(context *internal.Context) {
				defer wg.Done()
				...
				if sha != context.Sha {
					...
					numTokens, err := internal.GetNumTokens(body)
					if err != nil {
					...
					req[context.Id] = &internal.UpdateContextParams{
						Body: body,
					}
				}
			}(context)

		} else if context.ContextType == internal.ContextDirectoryTreeType {
			wg.Add(1)
			go func(context *internal.Context) {
				defer wg.Done()
		...
		} else if context.ContextType == internal.ContextURLType {
			wg.Add(1)
			go func(context *internal.Context) {
				defer wg.Done()
	...
	if len(req) == 0 && len(deleteIds) == 0 {
		...
		if len(deleteIds) > 0 {
			req, apiErr := api.Client.DeleteContext(CurrentPlanId, CurrentBranch, internal.DeleteContextRequest{
				Ids: deleteIds,
			})
			if apiErr != nil {
	...
	var removedContexts []*internal.Context
	for id := range deleteIds {
...
func tableForContextOutdated(updatedContexts []*internal.Context, tokenDiffsById map[string]int) string {
