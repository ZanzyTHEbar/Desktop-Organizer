...
func (m changesUIModel) getReplacementOldDisplay() oldReplacementRes {
	...
	if m.selectionInfo.currentRes.ReplaceWithLineNums {
		originalFile = internal.AddLineNums(originalFile)
	}
	...
	if m.selectionInfo.currentRep.EntireFile {
		...
		fileIdx = internal.IndexRunes(originalFileRunes, oldContentRunes)
		if fileIdx == -1 {
	...
	toPrepend = internal.RemoveLineNums(toPrepend)
	...
	toAppend = internal.RemoveLineNums(toAppend)

	wrapWidth := m.changeOldViewport.Width - 6
	toPrepend = wrap.String(toPrepend, wrapWidth)
	oldContent = internal.RemoveLineNums(oldContent)
