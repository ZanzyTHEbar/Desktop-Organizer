...
func printCmds(w io.Writer, prefix string, colors []color.Attribute, cmds ...string) {
	if os.Getenv("DesktopCleaner_DISABLE_SUGGESTIONS") != "" {
		return
	}
	for _, cmd := range cmds {
		...
		styled := color.New(colors...).Sprintf(" DesktopCleaner %s ", cmd)
...
func PrintCustomCmd(prefix, cmd, alias, desc string) {
	cmd = strings.Replace(cmd, alias, fmt.Sprintf("(%s)", alias), 1)
	// desc += color.New(color.FgWhite).Sprintf(" • alias → %s", color.New(color.Bold).Sprint(alias))
	styled := color.New(color.Bold, color.FgHiWhite, color.BgCyan, color.FgHiWhite).Sprintf(" DesktopCleaner %s ", cmd)
	fmt.Printf("%s%s 👉 %s\n", prefix, styled, desc)
}

// PrintCustomHelp prints the custom help output for the DesktopCleaner CLI
func PrintCustomHelp(all bool) {
	builder := &strings.Builder{}

	color.New(color.Bold, color.BgGreen, color.FgHiWhite).Fprintln(builder, " Usage ")
	color.New(color.Bold).Fprintln(builder, "  desktopcleaner [command] [flags]")
	color.New(color.Bold).Fprintln(builder, "  dc [command] [flags]")
	fmt.Fprintln(builder)

	color.New(color.Bold, color.BgGreen, color.FgHiWhite).Fprintln(builder, " Help ")
	color.New(color.Bold).Fprintln(builder, "  desktopcleaner help")
	color.New(color.Bold).Fprintln(builder, "  desktopcleaner help --all # show all commands")
	color.New(color.Bold).Fprintln(builder, "  desktopcleaner [command] --help")
	fmt.Fprintln(builder)

	color.New(color.Bold, color.BgMagenta, color.FgHiWhite).Fprintln(builder, " Getting Started ")
	fmt.Fprintln(builder)
	fmt.Fprintf(builder, "  1 - Create a new plan in your project's root directory with %s\n", color.New(color.Bold, color.BgCyan, color.FgHiWhite).Sprint(" desktopcleaner new "))
	fmt.Fprintln(builder)
	fmt.Fprintf(builder, "  2 - Load any relevant context with %s\n", color.New(color.Bold, color.BgCyan, color.FgHiWhite).Sprint(" desktopcleaner load [file-path-or-url] "))
	fmt.Fprintln(builder)
	fmt.Fprintf(builder, "  3 - Describe a task, ask a question, or chat with %s\n", color.New(color.Bold, color.BgCyan, color.FgHiWhite).Sprint(" desktopcleaner tell "))
	fmt.Fprintln(builder)
	...
	if all {
		...
		color.New(color.Bold, color.BgHiBlue, color.FgHiWhite).Fprintln(builder, " Use 'desktopcleaner help --all' or 'desktopcleaner help -a' for a list of all commands ")
